...
class ViewController: UIViewController ,UISearchBarDelegate,UITableViewDataSource,UITableViewDelegate,SFSafariViewControllerDelegate{
    ...
    var okashiList : [(name:String , maker:String , link:URL , image:URL)] = []//タプルを配列
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {//cellの値設定
        ...
        cell.textLabel?.text = okashiList[indexPath.row].name//行番号と名前の取得
        if let imageData = try? Data(contentsOf: okashiList[indexPath.row].image) {//簡易エラーハンドリング
